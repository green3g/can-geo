...
test('initMap with custom layers', (assert) => {
    ...
    assert.equal(vm.attr('mapObject').getLayers().getArray().length, 2,
    'two layers should be added to the map');
});
...
test('map event setCenter', (assert) => {
    ...
    const mercator = [0, 0];
    const expected = ol.proj.transform(mercator, 'EPSG:3857', 'EPSG:4326');
    vm.attr('mapObject').getView().setCenter(mercator);

    setTimeout(() => {
        assert.ok(
      areCloseEnough([vm.attr('x'), vm.attr('y')], expected),
      'the coordinates should approximately match the map objects center');
        done();
    }, 2500);
});
...
test('getTransformedCoordinates(x, y)', (assert) => {
    vm.initMap(element[0]);
    vm.attr({
        x: 10,
        y: 10
    ...
    assert.deepEqual(vm.getTransformedCoordinates(10, 10), ol.proj.transform([10, 10], 'EPSG:4326', 'EPSG:3857'),
